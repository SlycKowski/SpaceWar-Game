...
explosionIMG = []
for i in range(1,21):
    image = pygame.image.load(f'assets/explosion_{i}.png')
...
while not endGame:
    ...
    if keys[pygame.K_w]:
        if BlueShipTorpedoCount > 0:
            if len(BlueShipTorpedoes) == 0 or BlueShipTorpedoes[-1].lifetime < 350:  # Fire rate limit
            ...
            print(BlueShipTorpedoes[-1].lifetime)
    if keys[pygame.K_s]:
        if BlueShipFuel >= 1:
            BlueShipFuel -= 1
